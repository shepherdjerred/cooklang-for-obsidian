...
function parsePlainTextBody(body) {
  ...
  for (const line of body.split("\n")) {
    ...
    if (/ingredients/i.test(currentSection.name)) {
      ...
      const ingMatch = /^([\d½¼¾⅓⅔⅛⅜⅝⅞/\s.-]+)?((?:ounces?|oz|pounds?|lbs?|cups?|tablespoons?|tbsp|teaspoons?|tsp|cloves?|medium|large|small|pieces?|slices?|cans?|grams?|g|ml|liters?|pinch(?:es)?|heads?|bunche?s?|stalks?|sprigs?|inches?|dashes?)[.)]*\s+)?(.+)/i.exec(
        trimmed
      );
      if (ingMatch) {
